...
router.get("/", async (req, res) => {
  try {
    ...
    const data = await ToyModel.find(filterFind, null, { timeout: 100000 })
      .limit(perPage)
      .skip(page * perPage)
      .sort({ [sort]: reverse });
    res.json(data);
...
router.get("/cat", async (req, res) => {
  const data = req.query.c;
  const searchExp = new RegExp(data, "i");
  const filterData = await ToyModel.find({ category: searchExp }, null, { timeout: 100000 });
  res.json(filterData);
});
...
router.get("/price", async (req, res) => {
  ...
  try {
    const data = await ToyModel.find({ price: { $gte: min, $lte: max } }, null, { timeout: 100000 } );
    res.json(data);
  } catch (err) {
    console.log(err);
    res.status(502).json({ err });
...
router.get("/single/:id", async (req, res) => {
  try {
    ...
    const data = await ToyModel.findOne({ _id: id }, null, { timeout: 100000 });
    res.json(data);
  } catch (err) {
    console.log(err);
    res.status(502).json({ err });
